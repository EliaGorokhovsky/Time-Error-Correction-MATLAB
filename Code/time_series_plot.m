% This was originally part of a series of trial development scripts. However, it has been converted into
% a script that just plots a section of the time series to generate figure 1 for the paper.
...
obs_intvl =60;
% Eventually will want correlated observation errors, but not quite yet
obs_err_var = 1.0;
...
for t = 2:num_times + 1
   ...
   % Compute a linaerized approximation to the true_obs to keep track of how nonlinear things are
...
for inflation_indx = 1:7
%for inflation_indx = 3:3 
   inflate = poss_inflation(inflation_indx) 
   for location_indx = 2:5
   %for location_indx = 4:4
      half_width = poss_loc(location_indx)
...
%%%plot(real_offset);
%%%hold on
%%%plot(impossible_mn_offset, 'g');
%%%plot(mn_offset, 'r');

%%%legend('real_offset', 'impossible', 'possible');
